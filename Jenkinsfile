//OKD portion
def okdname = "core-f00183-nf-to-p3"
def okdnamespace = "core-f00183-nf-to-p3"
def okdbuildconfig = "core-f00183-nf-to-p3-nf-detection-event-v3"
def okdisname = "core-f00183-nf-to-p3-nf-detection-event-v3"
...
pipeline {
	...
	stages {
		stage('build') {
			...
			steps {
				sh """
					oc patch bc ${okdbuildconfig} -n ${okdnamespace} \
						-p '{
								"spec":
								{
									"source": {
